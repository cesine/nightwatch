...
var debug = require('debug')('nightwatch:child_process');
...
ChildProcess.prototype.run = function(colors, done) {
  ...
  setTimeout(function() {
    ...
    // TODO unable to find the test despite it matching
    // var label = self.env_label.trim().replace(/ /g, '').replace(/\//g,'');
    // var testFilePath = self.args.filter(function(arg) {
    //   arg = arg.toString().replace(/\//g,'');
    //   return arg.indexOf(label) > -1;
    // })[0];

    if (testFilePath.indexOf('node_modules') > -1){
      var paths = testFilePath.split('/');
      var module = paths[paths.indexOf('node_modules') + 1];
      var modleCWD = paths.splice(0, paths.indexOf('node_modules') + 2).join('/');
      console.warn('This test will use its own cwd ', module);
      testCWD = modleCWD;
    }
    debug('Spawning child_process in ' + testCWD, cliArgs);
    ...
    self.child.on('uncaughtException', function(err) {
      console.warn('uncaughtException', err, err.stack);
    });
