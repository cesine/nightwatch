var debug = require('debug')('nightwatch:run');
...
function processListener() {
  process.on('exit', function (code) {
    ...
    debug('globalResults', globalResults);
    if (exitCode === 0 && globalResults && (globalResults.errors > 0 || globalResults.failed > 0)) {
      exitCode = globalResults.errors || globalResults.failed;
    }
  ...
  Runner.readPaths(this.testSource, this.options)
    .spread(function(modulePaths, fullPaths) {
      ...
      (function runNextModule() {
        ...
        if (promise === null) {
          debug('promise for testResults was null');
          deferred.resolve();
          return;
        }

        promise.then(function(testResults) {
          debug('promise for testResults resolved', testResults);
